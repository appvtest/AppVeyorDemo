...
<xsl:stylesheet
  ...
  <xsl:param name="Create">
    ...
    <xsl:value-of select="/comparison/datasources/datasource[@id='1']/server" />.
    <xsl:value-of select="/comparison/datasources/datasource[@id='1']/database" />)
  </xsl:param>
  ...
  <xsl:param name="Drop">
    ...
    <xsl:value-of select="/comparison/datasources/datasource[@id='2']/server" />.
    <xsl:value-of select="/comparison/datasources/datasource[@id='2']/database" />)
  </xsl:param>
  ...
  <xsl:param name="Identical">
    <xsl:value-of select="$IdenticalCount" /> identical object
    <xsl:if test="$IdenticalCount > '1' or $IdenticalCount = '0'">s</xsl:if>
  </xsl:param>
...
.left_align {
    text-align: left;
        ...
        <script type="text/javascript">
          ...
          <!-- GET DIRECTION -->
          <xsl:text>
            var direction = &quot;</xsl:text><xsl:value-of select="@direction" /><xsl:text>&quot;;
          </xsl:text>
          <!-- GET TIMESTAMP -->
          <xsl:text>
            var timestamp = &quot;</xsl:text><xsl:value-of select="@timestamp" /><xsl:text>&quot;;
          </xsl:text>
          <xsl:apply-templates />
...
function createTables()
{
    ...
    for(j=0; j < onlyInOneRows.length; j++) 
    {
        ...
        robjnamecell = document.createElement("TD");
        robjnamecell.className =  "RObjNameDataRow";
        robjnametext = document.createTextNode(onlyInOneRows[j][3]);
        robjnamecell.appendChild(robjnametext);
        row.appendChild(robjnamecell);
...
//-->
      ...
      <body onLoad="pageInit()" onResize="doLayout()" style="padding: 0; margin: 0; overflow: hidden;"
            onmousemove="dragSqlDiv(event)" onmouseup="stopDrag(event)">
        <div id="datetime" class="DatabaseNameText"
             style="position:absolute; border-bottom: 1px solid #ebeadb;text-align:center; left:0px; top:0px; z-index:8; width:100%; height:24;">
        </div>
        <div style="position:absolute;top:25;width:100%">
          ...
          <div id="dbsvr" class="DatabaseNameText"
               style="position:absolute; top:8px; left:0px; width:44%; text-align:right">
          </div>
          <div id="dbname" class="DatabaseNameText"
               style="position:absolute; font-weight:bold; top:30px; left:0px; width:44%; text-align:right">
          </div>
          <div id="dbimg" class="DatabaseImage">
            <img id="dbimgimg" style="width:100%; height:100%" src="images/LeftDatabase32.gif" />
          </div>
          <div id="dbrsvr" class="DatabaseNameText"
               style="position:absolute; top:8px; left:55%; width:44%; text-align:left">
          </div>
          <div id="dbrname" class="DatabaseNameText"
               style="position:absolute; font-weight:bold; top:30px; left:55%; width:44%; text-align:left">
          </div>
  ...
  <xsl:template match="datasource">
    <!-- Datasource Type -->
    <xsl:text>var datasource</xsl:text><xsl:value-of select="@id" /><xsl:text>type = &quot;</xsl:text><xsl:value-of select="@type" /><xsl:text>&quot;;</xsl:text>
    <!-- Datasource Server -->
    <xsl:text>var datasource</xsl:text><xsl:value-of select="@id" /><xsl:text>server = &quot;</xsl:text><xsl:value-of select="redgate:replacequotes(server)" /><xsl:text>&quot;;</xsl:text>
    <!-- Datasource Database -->
    <xsl:text>var datasource</xsl:text><xsl:value-of select="@id" /><xsl:text>database = &quot;</xsl:text><xsl:value-of select="database" /><xsl:text>&quot;;</xsl:text>
  </xsl:template>
  ...
  <xsl:template name="processdifference">
    ...
    <xsl:call-template name="replace">
      <xsl:with-param name="pText" select="$node/object[@id=1]"/>
    ...
    <xsl:text>, &quot;</xsl:text>
    <xsl:call-template name="replace">
      <xsl:with-param name="pText" select="$node/object[@id=2]"/>
      <xsl:with-param name="pTarget" select='"\"'/>
      <xsl:with-param name="pReplacement" select='"\\"'/>
    </xsl:call-template>
    <xsl:text>&quot;, &quot;</xsl:text>
